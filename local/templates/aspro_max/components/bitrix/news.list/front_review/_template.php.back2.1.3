...
$arParams['SIZE_IN_ROW'] = $arParams['SIZE_IN_ROW'] ?? 1;
$arParams['COMPACT'] = $arParams['COMPACT'] ?? 'N';
$arParams['NOT_SLIDER'] = $arParams['NOT_SLIDER'] ?? '';
$arParams['LINKED_MODE'] = $arParams['LINKED_MODE'] ?? '';
...
<?if($arResult['ITEMS']):?>
	...
	<div class="maxwidth-theme only-on-front">
		...
		<div class="item-views reviews<?=($bCompact ? ' compact' : '');?><?=($bMoreItem ? ' more-item' : '');?> <?=($arParams['LINKED_MODE'] == 'Y' ? ' linked ' : '');?> <?=($notSlider ? ' list-mode ' : '');?>" itemscope itemtype="https://schema.org/Review">
			...
			<?else:?>
				<div class="owl-carousel owl-theme owl-bg-nav short-nav hidden-dots visible-nav swipeignore wsmooth1 appear-block loading_state<?=(!$bOneItem ? ' shadow' : '');?>" data-plugin-options='{"nav": true, "dots": false, "loop": false, "marginMove": true, "autoplay": false, <?=($animationSpeed >= 0 ? '"smartSpeed": '.$animationSpeed.',' : '')?> "useCSS": true, "responsive": {"0":{"items": 1, "autoWidth": true, "lightDrag": true, "margin":-1}, "601":{"items": 1, "autoWidth": true, "lightDrag": false, "margin":0}, "1200":{"items": <?=$col?>, "margin":-1}}}'>
			<?endif;?>
						...
						<div class="item clearfix <?=($bLogo ? ' wlogo' : '')?> <?=(!$bImage ? 'no_img' : '')?> <?=(!$bOneItem ? ' bordered box-shadow' : '');?> <?=($notSlider ? ' rounded2 bordered' : '');?>" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
							<div class="top_wrapper clearfix">
								...
								<div class="top-info">
									...
									<div itemprop="author" itemscope itemtype="https://schema.org/Person">
										<div itemprop="name" class="title <?=($bCompact ? 'font_md' : 'font_lg');?> pull-left">
											<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>"><?endif;?>
											<?=$arItem['NAME'];?>
											<?if($bDetailLink):?></a><?endif;?>
										</div>
							...
							<?if(isset($arItem['FIELDS']['PREVIEW_TEXT']) && $arItem['FIELDS']['PREVIEW_TEXT']):?>
								<div class="body-info">
									...
									<?endif;?>
									<?if(
										isset($arItem['DISPLAY_PROPERTIES']['FILE']) &&
										$arItem['DISPLAY_PROPERTIES']['FILE']['VALUE']
									):?>

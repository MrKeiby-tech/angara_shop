...
<?if($arResult):?>
	<?if (!function_exists('showSubItemss')) {
		function showSubItemss($arParams = [
			...
			<?endif;?>
			<span class="name <?=$arParams['WIDE_MENU'] ? 'option-font-bold' : ''?>"><?=$arParams['ITEM']["TEXT"]?></span><?=($arParams['ITEM']["CHILD"] && $arParams['SHOW_CHILDS'] ? CMax::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/trianglearrow_sprite.svg#trianglearrow_right", "right svg-inline-right", ['WIDTH' => 3,'HEIGHT' => 5, 'INLINE' => 'N']) : '')?>
			<?if($arParams['ITEM']["LINK"]):?>
				</a>
			<?endif;?>
		...
		<table>
			<tr>
				<?foreach($arResult as $arItem):?>
					...
					<td class="menu-item <?=($arItem["CHILD"] ? "dropdown" : "")?> <?=($bWideMenu ? 'wide_menu' : '');?> <?=(isset($arItem["PARAMS"]["CLASS"]) ? $arItem["PARAMS"]["CLASS"] : "");?>  <?=($arItem["SELECTED"] ? "active" : "")?>">
						<div class="wrap">
							...
							<?if($arItem["CHILD"] && $bShowChilds):?>
								...
								<?$bRightSideShow = ($arItem["BANNERS"] || $arResult["BRANDS"]) && $bWideMenu;?>
								<span class="tail"></span>

								<div class="dropdown-menu <?=$bRightSideShow ? 'with_right_block' : ''?> <?=$bManyItemsMenu ? 'long-menu-items' : ''?> <?=$arTheme['SHOW_RIGHT_SIDE']['DEPENDENT_PARAMS']['RIGHT_CONTENT']['VALUE']?>" >
									<?if($bManyItemsMenu):?>
													...
													<div class="menu-navigation__sections">
														<?foreach($arItem["CHILD"] as $arChild):?>
															<div class="menu-navigation__sections-item<?=($arChild['SELECTED'] ? " active" : "");?>">
																...
																>
																	<?if($arChild['CHILD']):?>
																		<?=CMax::showSpriteIconSvg(SITE_TEMPLATE_PATH."/images/svg/trianglearrow_sprite.svg#trianglearrow_right", "right svg-inline-right", ['WIDTH' => 3,'HEIGHT' => 5]);?>
																	<?endif;?>
																	<?if($bShowImg){?>
																		<span class="image colored_theme_svg ">
																			...
																			<?if(strpos($image, ".svg") !== false && \CMax::GetFrontParametrValue('COLORED_CATALOG_ICON') === 'Y'):?>
																				<?=\Aspro\Functions\CAsproMax::showSVG([
																					'PATH' => $image
																				]);?>
																			<?else:?>
									...
									<div class="customScrollbar scrollblock">
										...
										<?if($bRightSideShow && !$bManyItemsMenu):?>

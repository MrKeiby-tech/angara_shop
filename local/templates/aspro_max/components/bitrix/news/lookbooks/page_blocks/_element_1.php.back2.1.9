...
<?if($arElement['PROPERTY_LINK_GOODS_VALUE'] || $arElement['PROPERTY_SECTION_VALUE']):?>
<?// start catalog section block?>
		...
		<div class="ordered-block goods_catalog with-title <?=($bHideLeftBlock ? 'right_block wide_Y' : '')?>">
				<?$sTitle = $arElement["PROPERTY_H3_GOODS_VALUE"] ? $arElement["PROPERTY_H3_GOODS_VALUE"] : $arParams["T_GOODS"];?>
				<?if($sTitle):?>
		...
		<?$APPLICATION->AddViewContent('goods_catalog_block_prolog', $html);//?>
				...
				<?ob_start()//goods_catalog_block ?>
					...
					<?if( !empty($arElement['PROPERTY_LINK_GOODS_VALUE']) ):?>
						...
						<div class="catalog vertical filter_exists">
							...
							$GLOBALS[$arParams["FILTER_NAME"]]['ID'] = $arElement['PROPERTY_LINK_GOODS_VALUE'];
